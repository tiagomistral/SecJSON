...
function decrypt(json, options, callback) {
  ...
  try {
    ...
    var encryptionAlgorithm = stringify(doc.EncryptedData.EncryptionMethod);

    var decipher;
    var padding;
    var encryptedContent = stringify(doc.EncryptedData.CipherData.CipherValue);
...
function decryptKeyInfo(doc, options) {
  ...
  var keyInfo = stringify(doc.EncryptedData.KeyInfo.KeyInfo);
  var keyEncryptionAlgorighm = stringify(doc.EncryptedData.KeyInfo.EncryptedMethod.Algorithm);
  var encryptedKey = stringify(doc.EncryptedData.KeyInfo.CipherData.CipherValue);
...
function stringify(json) {
  ...
  return JSON.stringify(json).match(rePattern)[1];
}
