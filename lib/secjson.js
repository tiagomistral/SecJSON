...
function encryptKeyInfo(symmetricKey, options, callback) {
  ...
  switch (options.keyEncryptionAlgorighm) {
    case 'http://tiagomistral.github.io/SecJSON#rsa-oaep-mgf1p':
      return encryptKeyInfoWithScheme(symmetricKey, options, 'RSA-OAEP', callback)

    case 'http://tiagomistral.github.io/SecJSON#rsa-1_5':
...
function findKeyEncryptValue(json, keyPath, options){
  ...
  json[keyPath[0]] = findKeyEncryptValue(json[keyPath[0]], keyPath.slice(1, keyPath.length), options);
  return json;
}

function jsonSyncEncrypt(value, options){
  var json;
  encrypt(value, options, function(err, result) {
...
function findKeyDecryptValue(json, keyPath, options){
  ...
  json[keyPath[0]] = findKeyDecryptValue(json[keyPath[0]], keyPath.slice(1, keyPath.length), options);
  return json;
}

function jsonSyncDecrypt(value, options){
    var json;
    decrypt(value, options, function(err, result) {
      json = JSON.parse(result);
    });
    while(json === undefined) {
      deasync.runLoopOnce();
...
function decryptKeyInfo(doc, options) {
  ...
  var keyInfo = doc.EncryptedData.KeyInfo.EncryptedKey.KeyInfo.RetrievalMethod;
  var keyEncryptionAlgorighm = doc.EncryptedData.KeyInfo.EncryptedKey.EncryptionMethod.Algorithm;
  var encryptedKey = doc.EncryptedData.KeyInfo.EncryptedKey.CipherData.CipherValue;
